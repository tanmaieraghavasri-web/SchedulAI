...
const getAIClient = (manualKey?: string) => {
  ...
  const key = (manualKey || import.meta.env.VITE_GEMINI_API_KEY || (typeof process !== 'undefined' ? process.env?.VITE_GEMINI_API_KEY : '')).trim();

  if (!key || key.length < 5 || key === 'undefined' || key === '""') {
    return null;
  }

  try {
    // SDK requires an object with apiKey property
...
export default function App() {
  ...
  const perform3PhaseAnalysis = async (imgData: string) => {
    ...
    try {
      ...
      setError(`Analysis system failure: ${err instanceof Error ? err.message : 'Unknown'}`);
  ...
  const generatePlan = async () => {
    ...
    try {
      ...
      const cleanText = cleanJSON(response.text);
      setSchedule(JSON.parse(cleanText));
    } catch (err) {
